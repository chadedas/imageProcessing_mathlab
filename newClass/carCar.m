...
savePath = 'Car'; % กำหนดชื่อโฟลเดอร์ที่ต้องการบันทึกรูป
...
% Draw bounding boxes on the field
hold on;
rectangle('Position', carBoundingBox, 'EdgeColor', 'yellow', 'LineWidth', 2);
rectangle('Position', goalBoundingBox, 'EdgeColor', 'magenta', 'LineWidth', 2);
rectangle('Position', ballBoundingBox, 'Curvature', [1, 1], 'EdgeColor', 'cyan', 'LineWidth', 2);
hold off;

% Connect centers of bounding boxes with lines
hold on;
line([carBoundingBox(1) + carBoundingBox(3)/2, ballBoundingBox(1) + ballBoundingBox(3)/2], ...
     [carBoundingBox(2) + carBoundingBox(4)/2, ballBoundingBox(2) + ballBoundingBox(4)/2], ...
     'Color', 'white', 'LineWidth', 2);

line([ballBoundingBox(1) + ballBoundingBox(3)/2, goalBoundingBox(1) + goalBoundingBox(3)/2], ...
     [ballBoundingBox(2) + ballBoundingBox(4)/2, goalBoundingBox(2) + goalBoundingBox(4)/2], ...
     'Color', 'white', 'LineWidth', 2);
...
hold on;
text(fieldSizeX - 20, 60, ['มุมระหว่างรถและบอล: ' num2str(angle_car_ball) ' องศา'], 'Color', 'white', 'FontSize', 12, 'HorizontalAlignment', 'right');
text(fieldSizeX - 20, 80, ['มุมระหว่างบอลและประตู: ' num2str(angle_ball_goal) ' องศา'], 'Color', 'white', 'FontSize', 12, 'HorizontalAlignment', 'right');
...
text(fieldSizeX - 20, 20, ['ระยะห่างระหว่างรถและบอล: ' num2str(distanceCarBall) ' พิกเซล'], 'Color', 'white', 'FontSize', 12, 'HorizontalAlignment', 'right');
text(fieldSizeX - 20, 40, ['ระยะห่างระหว่างบอลและประตู: ' num2str(distanceBallGoal) ' พิกเซล'], 'Color', 'white', 'FontSize', 12, 'HorizontalAlignment', 'right');
...
% สร้างเฟรมสำหรับภาพเป็น GIF
gifFrames = cell(1, 1);

% ปรับค่า DelayTime สำหรับความเร็วในการเล่น GIF
delayTime = 0.1; % ปรับค่าตามต้องการ

% เพิ่มการเก็บภาพในเฟรมของ GIF ในทุกขั้นตอนของการเคลื่อนที่
gifFrames{end+1} = field;
...
% เคลื่อนที่ลูกบอลไปที่ประตูตามเส้นทางของรถ
while ~(abs(ballCenterX - goalCenterX) < 2 && abs(ballCenterY - goalCenterY) < 2) % ตรวจสอบว่าลูกบอลอยู่ใกล้ประตูหรือไม่
    % คำนวณทิศทางของลูกบอลไปยังประตู
    ballDirectionX = sign(goalCenterX - ballCenterX);
    ballDirectionY = sign(goalCenterY - ballCenterY);
    
    % เคลื่อนที่ลูกบอล
    randRowB = randRowB + ballSpeed * ballDirectionY;
    randColB = randColB + ballSpeed * ballDirectionX;
    
    % อัพเดทตำแหน่งของลูกบอล
    ballCenterX = randColB + size(ball, 2) / 2;
    ballCenterY = randRowB + size(ball, 1) / 2;
